...
func (d *GormDatabase) SyncWriter(body *model.SyncWriter) (*model.WriterClone, error) {
	...
	if writerClone.WriterThingClass == model.ThingClass.Point {
		point, _ := d.GetPoint(writerClone.WriterThingUUID, api.Args{WithPriority: true})
		if point != nil {
			priority := priorityarray.ConvertToMap(*point.Priority)
			pointModel := model.PointWriter{
				Priority: &priority,
			}
			_, _, _, _, _ = d.PointWrite(writerClone.WriterThingUUID, &pointModel, false, nil, true)
		}
	}
...
func (d *GormDatabase) SyncCOV(writerUUID string, body *model.SyncCOV) error {
	...
	if writer.WriterThingClass == model.ThingClass.Point {
		...
		_, _, _, _, err = d.PointWrite(uuid, &pointModel, false, nil, false)
		return err
	} else {
...
func (d *GormDatabase) SyncWriterWriteAction(sourceUUID string, body *model.SyncWriterAction) error {
	...
	if writerClone.WriterThingClass == model.ThingClass.Point {
		...
		_, _, _, _, err = d.PointWrite(producer.ProducerThingUUID, &pointWriter, false, &sourceUUID, false)
