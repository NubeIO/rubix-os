...
func CreatePointMetaTagsTransaction(tx *gorm.DB, pointUUID string, body []*model.PointMetaTag) ([]*model.PointMetaTag, error) {
	var keys []string
	for _, b := range body {
		var count int64
...
func (d *GormDatabase) CreatePointMetaTags(pointUUID string, body []*model.PointMetaTag) ([]*model.PointMetaTag, error) {
	tx := d.DB.Begin()
	mt, err := CreatePointMetaTagsTransaction(tx, pointUUID, body)
	if err != nil {
		tx.Rollback()
		return nil, err
	}
