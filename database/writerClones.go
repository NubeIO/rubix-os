...
func (d *GormDatabase) GetWriterCloneBySubUUID(consumerUUID string) (*model.WriterClone, error) {
	var wcm *model.WriterClone
	query := d.DB.Where("consumer_uuid = ? ", consumerUUID).First(&wcm); if query.Error != nil {
		return nil, query.Error
	}
	return wcm, nil
}
...
func (d *GormDatabase) UpdateWriterClone(uuid string, body *model.WriterClone) (*model.WriterClone, error) {
	var wcm *model.WriterClone
	...
	pro.CurrentWriterCloneUUID = uuid
