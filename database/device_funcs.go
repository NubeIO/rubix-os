...
func (d *GormDatabase) GetDeviceByPointUUID(pntUUID string) (*model.Device, error) {
	point, err := d.GetPoint(pntUUID, argspkg.Args{})
...
func (d *GormDatabase) GetOneDeviceByArgs(args argspkg.Args) (*model.Device, error) {
...
func (d *GormDatabase) GetDeviceByName(networkName string, deviceName string, args argspkg.Args) (*model.Device, error) {
	var deviceModel *model.Device
	query := d.buildDeviceQuery(args)
...
func (d *GormDatabase) deviceNameExistsInNetwork(deviceName, networkUUID string) (device *model.Device, existing bool) {
	network, err := d.GetNetwork(networkUUID, argspkg.Args{WithDevices: true})
	if err != nil {
		return nil, false
	}
...
func (d *GormDatabase) DeleteDeviceByName(networkName string, deviceName string, args argspkg.Args) (bool, error) {
	var deviceModel *model.Device
	query := d.buildDeviceQuery(args)
	if err := query.Joins("JOIN networks ON devices.network_uuid = networks.uuid").
		Where("networks.name = ?", networkName).Where("devices.name = ?", deviceName).
