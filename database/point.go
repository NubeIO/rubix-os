...
func (d *GormDatabase) GetPoints(args args.Args) ([]*model.Point, error) {
...
func (d *GormDatabase) GetPointsBulk(bulkPoints []*model.Point) ([]*model.Point, error) {
	var pointsModel []*model.Point
	points, err := d.GetPoints(args.Args{WithPriority: true})
	if err != nil {
		return nil, err
	}
...
func (d *GormDatabase) GetPoint(uuid string, args args.Args) (*model.Point, error) {
...
func (d *GormDatabase) GetPointByName(networkName, deviceName, pointName string, args args.Args) (*model.Point, error) {
	var pointModel *model.Point
	query := d.buildPointQuery(args)
...
func (d *GormDatabase) GetOnePointByArgs(args args.Args) (*model.Point, error) {
	var pointModel *model.Point
	query := d.buildPointQuery(args)
...
func (d *GormDatabase) UpdatePoint(uuid string, body *model.Point) (*model.Point, error) {
	...
	body.Name = name
	pointModel, err := d.GetPoint(uuid, args.Args{WithPriority: true})
	if err != nil {
		return nil, err
	}
...
func (d *GormDatabase) PointWriteByName(networkName, deviceName, pointName string, body *model.PointWriter) (*model.Point, error) {
	point, err := d.GetPointByName(networkName, deviceName, pointName, args.Args{})
	if err != nil {
		return nil, err
	}
...
func (d *GormDatabase) DeleteOnePointByArgs(args args.Args) (bool, error) {
	var pointModel *model.Point
	query := d.buildPointQuery(args)
	if err := query.First(&pointModel).Error; err != nil {
		return false, err
...
func (d *GormDatabase) DeletePointByName(networkName, deviceName, pointName string, args args.Args) (bool, error) {
	var pointModel *model.Point
	query := d.buildPointQuery(args)
	if err := query.Joins("JOIN devices ON points.device_uuid = devices.uuid").
		Joins("JOIN networks ON devices.network_uuid = networks.uuid").
