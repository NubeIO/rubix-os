...
func (d *GormDatabase) GetHost(uuid string, args parentArgs.Args) (*model.Host, error) {
...
func (d *GormDatabase) GetHostByName(name string) (*model.Host, error) {
	hostModel := model.Host{}
	query := d.buildHostQuery(parentArgs.Args{Name: &name})
...
func (d *GormDatabase) GetHosts(withOpenVPN bool, args parentArgs.Args) ([]*model.Host, error) {
	var hostsModel []*model.Host
	query := d.buildHostQuery(parentArgs.Args{})
...
func (d *GormDatabase) GetFirstHost(args parentArgs.Args) (*model.Host, error) {
...
func (d *GormDatabase) GetHostsByUUIDs(uuids []*string, args parentArgs.Args) ([]*model.Host, error) {
...
func (d *GormDatabase) ResolveHost(uuid string, name string) (*model.Host, error) {
	...
	if uuid != "" {
		host, _ := d.GetHost(uuid, parentArgs.Args{})
		if host != nil {
			return host, nil
		}
	}
	...
	var count int
	hosts, err := d.GetHosts(false, parentArgs.Args{})
	if err != nil {
		return nil, err
	}
	for _, h := range hosts {
