...
func (inst *Instance) BACnetServerPolling() error {
	...
	f := func() (bool, error) {
		...
		var err error
		nets, _ := inst.db.GetNetworksByPlugin(inst.pluginUUID, argspkg.Args{WithDevices: true})
		if len(nets) == 0 {
			...
			if boolean.IsFalse(net.Enable) {
				...
				if net.UUID != "" && net.PluginConfId == inst.pluginUUID {
					...
					for _, dev := range net.Devices { // DEVICES
						...
						dev, err = inst.db.GetDevice(dev.UUID, argspkg.Args{WithPoints: true})
						if err != nil {
...
func (inst *Instance) SyncFFPointWithBACnetServerPoint(pnt *model.Point, devUUID, netUUID string, forceWrite bool) (*model.Point, error) {
	pnt, err := inst.db.GetPoint(pnt.UUID, argspkg.Args{WithPriority: true})
	if err != nil {
