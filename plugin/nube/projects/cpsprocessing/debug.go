...
import (
	"fmt"
	"github.com/NubeIO/nubeio-rubix-lib-models-go-legacy/pkg/v1/model"
	"github.com/NubeIO/rubix-os/utils/nstring"
	log "github.com/sirupsen/logrus"
)

func (inst *Instance) cpsDebugMsg(args ...interface{}) {
...
func (inst *Instance) cpsErrorMsg(args ...interface{}) {
	prefix := "CPS: "
...
func (inst *Instance) printPointDebugInfo(pnt *model.Point) {
	if nstring.InEqualIgnoreCase(inst.config.LogLevel, "DEBUG") { // Added here to disable debug processes when not using logging
		...
		if pnt != nil {
			...
			inst.cpsDebugMsg(printString)
			return
		}
		inst.cpsDebugMsg("ERROR: INVALID POINT")
	}
