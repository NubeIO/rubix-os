...
func (inst *Instance) wizardTCP(body wizard) (string, error) {
	...
	case 1:
		...
		if err != nil {
			inst.tmvErrorMsg(fmt.Sprintf("network creation failure: %s", err))
		}
		inst.tmvDebugMsg("Created a Network")
		...
		for j := 1; j < 4; j++ {
			...
			if err != nil {
				inst.tmvErrorMsg(fmt.Sprintf("device creation failure: %s", err))
			}
			inst.tmvDebugMsg("Created a Device: ", dev)
			...
			if err != nil {
				inst.tmvErrorMsg(fmt.Sprintf("consumer point creation failure: %s", err))
			}
			inst.tmvDebugMsg("Created a Point for Consumer", pnt)
	...
	case 5:
		if body.NameArg != "" && body.AddArg > 0 {
			...
			if err != nil || net == nil {
				...
				inst.tmvDebugMsg("Network already exists")
			}
			...
			if err != nil || dev == nil || !exists {
				...
				inst.tmvDebugMsg("Device already exists")
			}
			...
			for _, point := range pointsArray {
				...
				if err != nil {
					inst.tmvErrorMsg(fmt.Sprintf("point creation failure: %s", err))
				}
				inst.tmvDebugMsg("Created a Point for Consumer", pnt)
			}
