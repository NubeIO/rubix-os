...
func (inst *Instance) addNetwork(body *model.Network) (network *model.Network, err error) {
	...
	if err != nil {
		inst.tmvErrorMsg("addNetwork(): failed to create modbus network: ", body.Name)
...
func (inst *Instance) addDevice(body *model.Device) (device *model.Device, err error) {
	...
	if device == nil || err != nil {
		inst.tmvDebugMsg("addDevice(): failed to create modbus device: ", body.Name)
...
func (inst *Instance) addPoint(body *model.Point) (point *model.Point, err error) {
	...
	if point == nil || err != nil {
		inst.tmvDebugMsg("addPoint(): failed to create modbus point: ", body.Name)
	...
	if err != nil || dev == nil {
		inst.tmvDebugMsg("addPoint(): bad response from GetDevice()")
		return nil, err
	}

	netPollMan, err := inst.getNetworkPollManagerByUUID(dev.NetworkUUID)
	if netPollMan == nil || err != nil {
		inst.tmvDebugMsg("addPoint(): cannot find NetworkPollManager for network: ", dev.NetworkUUID)
		return
	}
...
func (inst *Instance) updateNetwork(body *model.Network) (network *model.Network, err error) {
	...
	if netPollMan == nil || err != nil {
		inst.tmvDebugMsg("updateNetwork(): cannot find NetworkPollManager for network: ", network.UUID)
		return
	}
...
func (inst *Instance) updateDevice(body *model.Device) (device *model.Device, err error) {
	...
	if netPollMan == nil || err != nil {
		inst.tmvDebugMsg("updateDevice(): cannot find NetworkPollManager for network: ", device.NetworkUUID)
		return
	}
	if boolean.IsFalse(device.Enable) && netPollMan.PollQueue.CheckIfActiveDevicesListIncludes(device.UUID) {
		...
		if err != nil {
			inst.tmvDebugMsg("updateDevice(): error on ClearErrorsForAllPointsOnDevice(): ", err)
		}
...
func (inst *Instance) updatePoint(body *model.Point) (point *model.Point, err error) {
	...
	/*
		...
		if pnt == nil || err != nil {
			inst.tmvErrorMsg("could not find pointID: ", pp.FFPointUUID)
	...
	*/
	...
	dev, err := inst.db.GetDevice(point.DeviceUUID, api.Args{})
	if err != nil || dev == nil {
		inst.tmvDebugMsg("updatePoint(): bad response from GetDevice()")
		return nil, err
	}

	netPollMan, err := inst.getNetworkPollManagerByUUID(dev.NetworkUUID)
	if netPollMan == nil || err != nil {
		inst.tmvDebugMsg("updatePoint(): cannot find NetworkPollManager for network: ", dev.NetworkUUID)
...
func (inst *Instance) writePoint(pntUUID string, body *model.PointWriter) (point *model.Point, err error) {
	...
	/*
		...
		if err != nil || point == nil {
			inst.tmvErrorMsg("writePoint(): bad response from GetPoint(), ", err)
			return nil, err
		}
		...
		if !isWriteable(point.WriteMode, point.ObjectType) { // if point isn't writeable then reset writeable properties and do `UpdatePoint()`
			...
			if err != nil || point == nil {
				inst.tmvDebugMsg("writePoint(): bad response from UpdatePoint() err:", err)
				return nil, err
			}
			return point, nil
	*/
	...
	dev, err := inst.db.GetDevice(point.DeviceUUID, api.Args{})
	if err != nil || dev == nil {
		inst.tmvDebugMsg("writePoint(): bad response from GetDevice()")
		return nil, err
	}

	netPollMan, err := inst.getNetworkPollManagerByUUID(dev.NetworkUUID)
	if netPollMan == nil || err != nil {
		inst.tmvDebugMsg("writePoint(): cannot find NetworkPollManager for network: ", dev.NetworkUUID)
		...
		if isWriteValueChange { // if the write value has changed, we need to re-add the point so that it is polled asap (if required)
			...
			if pp == nil {
				if netPollMan.PollQueue.OutstandingPollingPoints.GetPollingPointIndexByPointUUID(point.UUID) > -1 {
					...
					inst.tmvDebugMsg("writePoint(): cannot find PollingPoint for point (could be out for polling: ", point.UUID)
...
func (inst *Instance) deleteNetwork(body *model.Network) (ok bool, err error) {
	...
	if !found {
		inst.tmvDebugMsg("deleteNetwork(): cannot find NetworkPollManager for network: ", body.UUID)
	}
...
func (inst *Instance) deleteDevice(body *model.Device) (ok bool, err error) {
	...
	if netPollMan == nil || err != nil {
		inst.tmvDebugMsg("deleteDevice(): cannot find NetworkPollManager for network: ", body.NetworkUUID)
...
func (inst *Instance) pointUpdate(point *model.Point, value float64, readSuccess, clearFaults bool) (*model.Point, error) {
	...
	if err != nil {
		inst.tmvDebugMsg("MODBUS UPDATE POINT UpdatePointPresentValue() error: ", err)
		return nil, err
	}
	return point, nil
}
