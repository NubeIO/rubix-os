...
func (inst *Instance) ProcessQueryParams(jobConfig Job) (string, error) {
	...
	/*
		...
		var validHighLimit bool


		if jobConfig.HighLimitThreshold == nil && jobConfig.LowLimitThreshold == nil {
			return "", errors.New("no high or low limit thresholds were set")
		}

		thresholdParams := ""

		if jobConfig.HighLimitThreshold != nil {
			highLimit = *jobConfig.HighLimitThreshold
			validHighLimit = true
			thresholdParams += fmt.Sprintf("value>=%f", highLimit)
		}
		if jobConfig.LowLimitThreshold != nil {
			lowLimit = *jobConfig.LowLimitThreshold
			if validHighLimit {
				thresholdParams += "%26%26"
			}
			thresholdParams += fmt.Sprintf("value<=%f", lowLimit)
		}
		inst.thresholdalertsDebugMsg("ProcessQueryParams() thresholdParams: ", thresholdParams)
