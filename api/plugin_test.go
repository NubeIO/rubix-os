...
import (
	...
	"github.com/NubeDev/plug-framework/plugin/testing/mock"
...
func (s *PluginSuite) BeforeTest(suiteName, testName string) {
	...
	s.db.CreatePluginConf(&model.PluginConf{
		UserID:     1,
		ModulePath: "github.com/NubeDev/plug-framework/plugin/example/removed",
		Token:      "P1234",
		Enabled:    false,
	})
}
...
func (s *PluginSuite) getDanglingConf(uid uint) *model.PluginConf {
	conf, err := s.db.GetPluginConfByUserAndPath(uid, "github.com/NubeDev/plug-framework/plugin/example/removed")
