...
type MemberDatabase interface {
	GetMembers(args parentArgs.Args) ([]*model.Member, error)
	GetMember(uuid string, args parentArgs.Args) (*model.Member, error)
	GetMemberByUsername(username string, args parentArgs.Args) (*model.Member, error)
	GetMemberByEmail(email string, args parentArgs.Args) (*model.Member, error)
...
func (a *MemberAPI) Login(ctx *gin.Context) {
	...
	member, _ := a.DB.GetMemberByUsername(body.Username, parentArgs.Args{})
...
func (a *MemberAPI) CheckEmail(ctx *gin.Context) {
	...
	q, _ := a.DB.GetMemberByEmail(email, parentArgs.Args{})
...
func (a *MemberAPI) CheckUsername(ctx *gin.Context) {
	...
	q, _ := a.DB.GetMemberByUsername(username, parentArgs.Args{})
...
func (a *MemberAPI) VerifyMember(ctx *gin.Context) {
	username := resolveUsername(ctx)
	member, err := a.DB.GetMemberByUsername(username, parentArgs.Args{})
	if err != nil {
		ResponseHandler(nil, err, ctx)
		return
	}
